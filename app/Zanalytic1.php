...
class Zanalytic1 extends Model {
	...
	public static function analytic(){
		...
		$acounts=array();
		$arows=DB::table('zanalytic')->where('price0change','>=',$p)->where('date','=',$date)->get();
		...
		foreach($zanalytics as $zanalytic){
			if($zanalytic->vrate1<40 and $zanalytic->vrate<40 and isset($azstocks[$zanalytic->zstock_id])){
				$azstocks[$zanalytic->zstock_id]+=round(5*$zanalytic->vrate/10)+round(3*$zanalytic->vrate1/10);
			}
		}
