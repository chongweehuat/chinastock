...
class Zstudy1 extends Model {
	...
	public static function bestlist(){
		...
		$sql="select p1,p2,sum(cprofit) as n from zdstudy where date>='{$mdate}' and p2<=6 group by p1,p2 order by n desc limit 20";
		...
		$zdstudy=DB::select("select p1,p2,sum(hprofit) as n from zdstudy where date>='{$mdate}' and p2<=6 group by p1,p2 order by n desc limit 20");
